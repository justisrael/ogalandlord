...
export const createAdminDocument = async ({ email, passwordHash, fullName, roles = [] }) => {
  try {
    const id = uuid();
    const docRef = doc(db, "admins", id);
    const adminData = { id, email, passwordHash, fullName, roles, createdAt: new Date().toISOString() };
...
export const updateAdminDocument = async (admin) => {
  try {
...
export const saveAuditLog = async ({ action, adminId, adminName, adminEmail, details = {} }) => {
  try {
    const id = uuid();
