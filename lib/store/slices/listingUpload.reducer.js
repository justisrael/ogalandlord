...
export const getPaymentLink = createAsyncThunk(
  ...
  async (cartDetails, { rejectWithValue }) => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/acceptpayment`;
    ...
    try {
      ...
      const link = responseData.data.authorization_url;
      // console.log(link)
      fallback();
      return link;
    } catch (error) {
...
export const verifypayments = createAsyncThunk(
  ...
  async (ref, { rejectWithValue, dispatch, getState }) => {
    const url = `${process.env.NEXT_PUBLIC_API_URL}/verify?reference=${ref}`;
    try {
      const { data } = await axios.get(url);
      const responseData = typeof data === "string" ? JSON.parse(data) : data;
      if (responseData.data.status === "success") {
        const metadata = responseData.data.metadata.custom_fields;
        ...
        const paidAt = responseData.data.paidAt;
        const reference = ref;
